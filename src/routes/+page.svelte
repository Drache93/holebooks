<script lang="ts">
    ...
    const reading = $derived(
        data.reading.filter((b) => hits(b.title, b.author)),
    );
    const read = $derived(data.read.filter((b) => hits(b.title, b.author)));
    const planned = $derived(
        data.planned.filter((b) => hits(b.title, b.author)),
    );

    const goalPct = $derived(
        Math.min(100, Math.round((data.stats.read / data.stats.goal) * 100)),
    );
    ...
    const nextYear = $derived(
        yearIndex < data.years.length - 1 ? data.years[yearIndex + 1] : null,
    );
        ...
        <div class="wrap stats-content">
            <div class="stats-row">
                <div class="stats-counts">
                    ...
                    <div class="stat">
                        <span class="stat-n"
                            >{data.stats.pagesRead.toLocaleString()}</span
                        >
            ...
            <div class="goal-row">
                ...
                <span class="goal-label"
                    >{data.stats.read} of {data.stats.goal} books</span
                >
    ...
    <main class="wrap main-content">
        ...
        {#if reading.length > 0}
            <section class="section anim d3">
                <h2 class="section-label">
                    <span class="label-dot" style="background:var(--accent)"
                    ></span>
                    Currently Reading
                </h2>
                    ...
                    {#each reading as book (book.id)}
                        ...
                        <a href="/book/{book.id}" class="rc">
                            ...
                            <div class="rc-info">
                                ...
                                {#if book.genre}<span class="rc-genre"
                                        >{book.genre}</span
                                    >{/if}
        ...
        <section class="section anim d4">
            <h2 class="section-label">
                ...
                Read in {data.year}
                {#if read.length > 0}<span class="label-badge"
                        >{read.length}</span
                    >{/if}
            </h2>
            <div class="year-grid-outer">
                ...
                <div class="year-grid-inner">
                    ...
                    {:else}
                        <p class="year-empty">
                            {q
                                ? `No matches in ${data.year}`
                                : `No books finished in ${data.year}`}
                        </p>
                    {/if}
                </div>
        ...
        {#if planned.length > 0}
            <section class="section anim d4">
                <h2 class="section-label">
                    <span class="label-dot" style="background:var(--text-3)"
                    ></span>
                    Want to Read
                ...
                <div class="grid">
                    {#each planned as book (book.id)}
                        ...
                        <a href="/book/{book.id}" class="tile planned">
                            ...
                            <p class="tile-title">{book.title}</p>
                            <p class="tile-author">{book.author}</p>
                            {#if book.totalPages}<p class="tile-pp">
                                    {book.totalPages} pp
                                </p>{/if}
                        </a>
                    {/each}
...
<style>
    ...
    .tile.planned .tile-cover {
        opacity: 0.72;
    }
    .tile.planned:hover .tile-cover {
        opacity: 1;
    }
